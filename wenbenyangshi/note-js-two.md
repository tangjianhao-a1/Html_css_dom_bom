...
substr(strt,length)|从strt位置开始(索引号)，length取得个数，重点记住这个   
slice(strt,end)|从strt位置开始，截取到end位置，end取不到(他们俩都是索引号)
substring(strt,end)|从start位置开始，截取到end位置，end取不到，基本和slice相同，但是不接受负值，   
...
## 3.简单类型的内存分配    
- 值类型（简单的数据类型）：string,number,boolean,undefined,null   
